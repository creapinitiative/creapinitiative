...
export const Route = createFileRoute("/resources/press-statements")({
  head: () => ({
    meta: [
      { title: "Press Statements — CREAP Africa Initiative" },
      { name: "description", content: "Official press statements from CREAP Africa Initiative." },
    ],
  }),
...
function PressStatementsPage() {
  ...
  return (
    <>
      <PageHero
        eyebrow="Home / Knowledge Hub / Press Statements"
        title="Press Statements"
        body="Official statements and public positions from CREAP Africa Initiative."
      />
      ...
      <section className="bg-g50 py-16 lg:py-24">
        <Reveal as="div" className="mx-auto max-w-[1300px] px-5 sm:px-8 md:px-12 lg:px-28 space-y-4">
          {items.length === 0 && (
            <p className="text-ink3 text-center py-16">No press statements published yet.</p>
          )}
          {items.map((item, idx) => (
            <RevealItem
              key={item.id}
            ...
            >
              ...
              <div className="flex-1">
                <p className="text-[11px] tracking-[0.14em] uppercase text-ink4 mb-2">Press Statement · {item.date}</p>
